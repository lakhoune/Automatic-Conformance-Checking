...
def _get_top_variants(connector):
    ...
    # get number of cases
    query = PQL()
    query += PQLColumn(name="case count", query=f""" FLOOR(COUNT (DISTINCT "{activity_table}"."{case_col}")*0.01) """)
    ...
    query.add(PQLColumn(name=timestamp, query=f""" "{activity_table}"."{timestamp}" """))
...
def _discover_petri_net_from_log(connector, events, evaluate=False):
    ...
    # discover model
    net, initial_marking, final_marking = discover_petri_net_inductive(filtered_dataframe)

    if evaluate:
        # evaluate discovered model (takes a while on big logs)
        fitness = pm4py.fitness_token_based_replay(df_formatted, net, initial_marking, final_marking)
        precision = pm4py.precision_token_based_replay(df_formatted, net, initial_marking, final_marking)
        gen = generalization_evaluator.apply(df_formatted, net, initial_marking, final_marking)
...
def alignment_scores(events_to_align, event_log, connector):
    ...
    events_to_align = pm4py.format_dataframe(events_to_align, case_id=connector.case_col(),
                                             ...
                                             timestamp_key=connector.timestamp())
    # discover model
    net, im, fm = _discover_petri_net_from_log(events=event_log, connector=connector, evaluate=False)
